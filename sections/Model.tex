...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
        ...
        \State{Operate with each move on the graph, generate all corresponding graphs}
        \State{Calculate $\Delta E$ for each allowed move, $E_D$, $E_C$ and $E_d$ on the selection set for each graph $\Delta E_D$, $\Delta E_C$, $\Delta E_d$  }
